    ...
    def execute(self, entity: User) -> Result:
        ...
        try:
            ...
            if 'user_email_key' in str(error):
                result.msg = f'You cannot update email={entity.email} because it already exists.'
            elif 'user_username_key' in str(error):
                result.msg = f'You cannot update username={entity.username} because it already exists.'
            else:
